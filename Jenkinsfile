    ...
    stage ('Deploy') {
      steps {
        script {
          def imageTag = "${IMAGE_TAG}"
          def imageName = "${IMAGE_NAME}:${imageTag}"
          def containerName = "fco"
          def containerPort = 80
          def applicationName = "AppECS-fcoweb-fco-web"
          def deploymentGroupName = "DgpECS-fcoweb-fco-web"
          def taskDefinitionName = "fco-web"
          ...
          aws ecs register-task-definition --region us-east-1 --family fco-web               \
